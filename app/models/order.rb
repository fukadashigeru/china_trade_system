...
class Order < ApplicationRecord
  ...
  # 今回の買付先を更新するメソッド
  def processing_actual_item_units_and_taobao_urls(taobao_url_params, first_candidate_params, have_stock_params, current_company)
    ...
    taobao_url_params.keys.each do |i|
      taobao_url_params[i].keys.each do |j|
        ...
        taobao_url_params[i][j].keys.each do |k|
          taobao_url_params[i][j][k].keys.each do |l|
            ...
            if l == "0"
              ...
              taobao_url = current_company.taobao_urls.find_or_create_by(url: taobao_url_params_url)
              actual_item_unit.actual_item_unit_taobao_urls.create(taobao_url_id: taobao_url.id)
              taobao_url_ids_included_in_params << taobao_url.id
            ## 上書きされたINPUTの場合
            else
              taobao_url = actual_item_unit.taobao_urls.find(l.to_i)
              ...
              if taobao_url.item_units.length + taobao_url.actual_item_units.length > 1
                if taobao_url_params_url.present?
                  taobao_url = current_company.taobao_urls.find_or_create_by(url: taobao_url_params_url)
                  actual_item_unit_taobao_url.update(taobao_url_id: taobao_url.id)
            ...
            if first_candidate_params
              if k == first_candidate_params[i]&.keys&.first
                actual_item_unit.update(first_candidate_id: taobao_url.id)
              end
            end
            # 在庫の有無を変更
            if taobao_url_params_url.present?
        ...
        #削除処理
        removed_taobao_url_ids = actual_item_unit.taobao_urls.ids - taobao_url_ids_included_in_params
        removed_taobao_url_ids.each do |i|
          removed_taobao_url = current_company.taobao_urls.find(i)
          if removed_taobao_url.actual_item_units.length > 1
            actual_item_unit_taobao_url = ActualItemUnitTaobaoUrl.find_by(actual_item_unit_id: actual_item_unit.id, taobao_url_id: i)
            actual_item_unit_taobao_url.destroy
          else
    ...
    removed_actual_item_unit_ids = actual_item_units.ids - actual_item_unit_ids_included_in_params
    removed_actual_item_unit_ids.each do |i|
      removed_actual_item_unit = actual_item_units.find(i)
      removed_actual_item_unit.destroy
    end
  end
